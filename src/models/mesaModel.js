...
const createMesa = async (mesa) => {
    const { id_zona, numero_mesa, capacidad } = mesa;
    const query = `INSERT INTO Mesas (id_zona, numero_mesa, capacidad) VALUES (?, ?, ?)`;
    const [result] = await db.execute(query, [id_zona, numero_mesa, capacidad]);
